...
(ert-deftest test-microtheory-context-isolation ()
  ...
  (in-microtheory Mt1
    (should (equal t (car (kb-query 'God 'exists)))))
  
  (in-microtheory Mt2
    (should (equal nil (car (kb-query 'God 'exists))))))
...
(ert-deftest test-microtheory-inheritance ()
  ...
  (in-microtheory ChildMt
    ...
    (should (member 'inherited (kb-query 'base 'fact)))))
...
(ert-deftest test-multiple-microtheory-parents ()
  ...
  (in-microtheory ChildMt
    (should (member 'parent1 (kb-query 'fact1 'source)))
    (should (member 'parent2 (kb-query 'fact2 'source)))))
